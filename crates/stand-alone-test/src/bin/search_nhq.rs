use std::path::Path;
use base::{index::{IndexOptions, NhqIndexingOptions, OptimizingOptions, SearchOptions, SegmentsOptions, VectorOptions}, scalar::F32, search::{Filter, Payload}, vector::Vecf32Borrowed};
...
fn main() {
    ...
    let nhq = nhq::mock_open(path, options);
    ...
    for i in 0..nq {
        ...
        let mut heap = nhq.basic(vector, &opts, filter.clone());
